...
const WIDGET_BASE = 'https://player-widget.mixcloud.com/widget/iframe/';
...
/**
 * Mixcloud embeds are driven by the cloudcast `key` (e.g. `/spartacus/party-time/`).
 * `light=1` gives the compact player that sits nicely under the artwork.
 */
export function buildEmbedUrl(key: string | undefined, autoplay: boolean): string | null {
  if (!key) return null;
...
export function mapCloudcast(raw: MixcloudCloudcast): Track | null {
  ...
  return {
    ...
    tags: (raw.tags ?? [])
      ...
      .slice(0, 3),
    embedUrl: buildEmbedUrl(raw.key, true),
  };
}
